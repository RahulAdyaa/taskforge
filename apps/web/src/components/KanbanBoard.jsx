...
export default function KanbanBoard({ projectId, tasks, isAdmin, members }) {
    ...
    <div className="flex h-full p-8 gap-8 overflow-x-auto bg-off-white">
      ...
      >
        {['TODO', 'IN_PROGRESS', 'DONE'].map(status => (
          <Column key={status} id={status} title={status.replace('_', ' ')} tasks={columns[status]} onTaskClick={setSelectedTask} />
        ))}
        <DragOverlay>
...
function TaskCardUI({ task, isOverlay }) {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-3">
        ...
        <span className="font-mono text-[10px] text-black/40 uppercase">ID: {task.id.slice(0,6)}</span>
      </div>
